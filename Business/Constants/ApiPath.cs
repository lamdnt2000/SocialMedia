namespace WebAPI.Constant
{
    public class ApiPath
    {
        public const string API_VERSION = "/v1";
        public const string ROLE_PATH = API_VERSION + "/role";
        public const string USER_PATH = API_VERSION + "/users";
        public const string ORAGANIZATION_PATH = API_VERSION + "/organizations";
        public const string BRAND_PATH = API_VERSION + "/brands";
        public const string PLATFORM_PATH = API_VERSION + "/platforms";
        public const string CATEGORY_PATH = API_VERSION + "/categories";
        public const string LOCATION_PATH = API_VERSION + "/locations";
        public const string CHANNEL_PATH = API_VERSION + "/channels";
        public const string REACTIONTYPE_PATH = API_VERSION + "/reactiontypes";
        public const string HASHTAG_PATH = API_VERSION + "/hashtags";
        public const string WALLET_PATH = API_VERSION + "/wallets";
        public const string GATEWAY_PATH = API_VERSION + "/gateways";
        public const string TRANSACTIONDEPOSIT_PATH = API_VERSION + "/transactiondeposits";
        public const string SUBCRIPTION_PATH = API_VERSION + "/subcriptions";
        public const string OFFER_PATH = API_VERSION + "/offers";
        public const string PACKAGE_PATH = API_VERSION + "/packages";
    }
}
